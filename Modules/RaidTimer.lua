...
function MonDKP:StopRaidTimer()
  ...
  adjustTab.RaidTimerContainer.OutputHeader:SetText(L["RAIDENDED"] .. ":")
  adjustTab.RaidTimerContainer.StartTimer:SetText(L["INITRAID"])
  adjustTab.RaidTimerContainer.Output:SetText("|cff00ff00" .. strsub(adjustTab.RaidTimerContainer.Output:GetText(), 11, -3) .. "|r")
  MonDKP.RaidTimerPopout.Output:SetText(adjustTab.RaidTimerContainer.Output:GetText());
  adjustTab.RaidTimerContainer.PauseTimer:Hide();
  adjustTab.RaidTimerContainer.BonusHeader:SetText(L["TOTALDKPAWARD"] .. ":")
  timer = 0;
  awards = 0;
  StartAwarded = false;
  MinuteCount = 0;
  ...
  if MonDKP:IsInRaid() and MonDKP:CheckRaidLeader() and core.IsOfficer then
    if MonDKP_DB.DKPBonus.GiveRaidEnd then -- Award Raid Completion Bonus
      ...
      adjustTab.RaidTimerContainer.Bonus:SetText("|cff00ff00" .. totalAwarded .. "|r")
    end
    totalAwarded = 0;
  elseif MonDKP:IsInRaid() and core.IsOfficer then
...
function MonDKP:StartRaidTimer(pause, syncTimer, syncSecondCount, syncMinuteCount, syncAward)
  ...
  if not syncTimer then
    if not pause then
      adjustTab.RaidTimerContainer.StartTimer:SetText(L["ENDRAID"])
      adjustTab.RaidTimerContainer.OutputHeader:SetText(L["TIMEELAPSED"] .. ":")
      adjustTab.RaidTimerContainer.OutputHeader:Show();
      if MonDKP_DB.DKPBonus.GiveRaidStart and not StartAwarded then
        ...
        if totalAwarded == 0 then
          adjustTab.RaidTimerContainer.Bonus:SetText("|cffff0000" .. totalAwarded .. "|r")
        else
          adjustTab.RaidTimerContainer.Bonus:SetText("|cff00ff00" .. totalAwarded .. "|r")
        end
      end
      adjustTab.RaidTimerContainer.BonusHeader:SetText(L["BONUSAWARDED"] .. ":")
      adjustTab.RaidTimerContainer.BonusHeader:Show()
      adjustTab.RaidTimerContainer.PauseTimer:Show();
      increment = MonDKP_DB.modes.increment;
      core.RaidInProgress = true
    else
      MonDKP.RaidTimer:SetScript("OnUpdate", nil)
      adjustTab.RaidTimerContainer.StartTimer:SetText(L["CONTINUERAID"])
      adjustTab.RaidTimerContainer.OutputHeader:SetText(L["RAIDPAUSED"] .. ":")
      adjustTab.RaidTimerContainer.PauseTimer:Hide();
      adjustTab.RaidTimerContainer.Output:SetText("|cffff0000" .. strsub(adjustTab.RaidTimerContainer.Output:GetText(), 11, -3) .. "|r")
      MonDKP.RaidTimerPopout.Output:SetText(adjustTab.RaidTimerContainer.Output:GetText())
      core.RaidInProgress = false
      return;
    end

    if MonDKP:IsInRaid() and MonDKP:CheckRaidLeader() and not pause and core.IsOfficer then
  ...
  MonDKP.RaidTimer:SetScript("OnUpdate", function(self, elapsed)
    ...
    if SecondTracker >= 1 then
      ...
      adjustTab.RaidTimerContainer.Output:SetText("|cff00ff00" .. curTicker .. "|r")
    ...
    if MinuteCount >= increment and increment > 0 then -- apply bonus once increment value has been met
      ...
      if MonDKP:IsInRaid() and MonDKP:CheckRaidLeader() then
