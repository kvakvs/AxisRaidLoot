if GetLocale() == "koKR" then
  ...
  core.L = {
    ...
    MIGRATEINST2 = "%s 은 (는) 이미 마이그레이션을 수행했습니다. 저장된 변수 파일을 즉시 백업하고 (WTF\\Account\\ACCOUNT_NAME\\SavedVariables\\AxisRaidLoot.lua) 아래 테이블 삭제 버튼을 클릭하십시오. %s 이 (가) 온라인 인 경우 다시로드하면 자동으로 동기화됩니다. 그렇지 않은 경우 로그인 할 때 푸시되거나 동기화를 푸시하도록 요청할 수 있습니다 (메인 GUI의 왼쪽 하단 모서리에있는 테이블 상태 단추를 클릭하여 수행). 길드 마이그레이션 프로세스에서 오류가 발생하면 길드 리더의 공개 메모에서 마이그레이션 관리자 태그를 삭제하고 지금까지 참여한 모든 임원이 손상된 저장된 변수 파일을 삭제하고 백업 한 것으로 복원하십시오. 이 작업이 완료 될 때까지 모든 사람은 동시에이 작업을 수행해야하며 애드온이 활성화 된 상태에서 로그인하지 않아야합니다. 마이그레이션 된 테이블이있는 온라인 사용자는 로그인하자마자 해당 데이터를 자동으로 푸시합니다.",
    MIN = "최소",
    ...
    임원이 필요한 권한을 제한하려면이 화이트리스트 설정 만 사용하는 것이 좋습니다. 모든 임원에게 권한을 부여하려면이 설정 기능을 완전히 무시하십시오. 주의해서 사용하십시오. (이 설정을 적용하면 문제가 발생하면 텍스트 편집기로 \WTF\Accounts\ACCOUNT_NAME\SavedVariables\AxisRaidLoot.lua 파일을 열고 하단 근처의 MonDKP_Whitelist 테이블을 삭제하십시오.)]=],
    WIPETABLES = "테이블 정리",
